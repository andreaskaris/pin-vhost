...
// Metadata for the eBPF kprobe of this program.
var progSpec = &ebpf.ProgramSpec{
	Name:    "trace_vhost_net", // non-unique name, will appear in `bpftool prog list` while attached
	Type:    ebpf.Kprobe,       // only TracePoint programs can be attached to trace events created by link.Tracepoint()
...
func main() {
	...
	// Open a kprobe for vhost_net_open.
	// Each time a vhost socket is opened (kernel function vhost_net_open()), the eBPF
	// program specified above will be executed and a '123' value will appear
	// in the perf ring.
	tp, err := link.Kprobe("vhost_net_ioctl", prog, nil)
	if err != nil {
